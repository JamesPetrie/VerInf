...
$$\underbrace{H(\mathrm{AES}(\text{key}, \boldsymbol{tok})) = H_1}_{\text{transcript anchor (Appendix E)}} \;\;\wedge\;\; \underbrace{\boldsymbol{\ell} = D(\boldsymbol{tok})}_{\text{forward pass (B.2 to B.6)}} \;\;\wedge\;\; \underbrace{-\textstyle\sum_{t} \log_2 Q(\boldsymbol{o}_t \mid \boldsymbol{\ell}_t) \;\le\; \hat{U}}_{\text{surprisal bound (B.7)}}$$

with $\boldsymbol{tok}$ the committed token streams, $H_1$ the digest recorded at generation time (alongside the key commitment $H_2$), $D$ the committed integer model of §3, $\boldsymbol{\ell}$ the logits it produces, with $\boldsymbol{\ell}_t$ the row scoring position $t$, $\boldsymbol{o}_t$ the committed output tokens, $Q$ the committed predictor of §4, and $\hat{U}$ the revealed value. The parts are held to different standards. The anchor and the forward pass must each leave the prover no freedom at all, the first by collision resistance and the second by constraint structure; the surprisal computation may leave freedom, provided every free direction inflates the reported value.
