...
missing trusted enrollment root, the production shape (a committed weight
...
def _run(path, root_w, stmt, wc_root):
    r = subprocess.run([_verify_proof_bin(), path, root_w, stmt, wc_root],
...
def test_bridged_proof_accepts_with_its_enrollment_root_and_rejects_without():
    tape, proof, _ = _prove_with_flag()
    path = _dump(tape, proof)
    try:
        root_w, stmt, wc_root = _policy(proof)
        acc, msg = _run(path, root_w, stmt, wc_root)
        assert acc, f"honest bridged proof rejected: {msg}"
        acc, msg = _run(path, root_w, stmt, WRONG)
        assert not acc and "enrollment root" in msg, f"wrong enrollment root accepted: {msg}"
        acc, msg = _run(path, root_w, stmt, "-")
        assert not acc, "no trusted enrollment root: expected REJECT"
        acc, msg = _run(path, wc_root, stmt, "-")
        assert not acc, "enrollment root in the weight-root slot: expected REJECT"
        print("    enrollment root: matching ACCEPT / wrong, missing, misplaced REJECT")
    finally:
        os.unlink(path)
...
def test_legacy_seed_path_never_feeds_the_bridge():
    ...
    try:
        ...
        root_w, _stmt, wc_root = _policy(proof)
        acc, msg = _run(path, root_w, "-", wc_root)
...
def test_production_shape_binds_both_anchors():
    ...
    root and left the enrollment root unbound; now each anchor has its slot
    and a wrong or missing enrollment root rejects beside a correct weight
    root."""
    ...
    try:
        ...
        acc, msg = _run(path, root_w, stmt, WRONG)
        assert not acc and "enrollment root" in msg, f"wrong enrollment root beside a right weight root accepted: {msg}"
        acc, msg = _run(path, root_w, stmt, "-")
        assert not acc, "missing enrollment root beside a right weight root: expected REJECT"
        acc, msg = _run(path, WRONG, stmt, wc_root)
        assert not acc, "wrong weight root beside a right enrollment root: expected REJECT"
