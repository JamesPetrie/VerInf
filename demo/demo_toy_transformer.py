"""Ultra-small toy transformer-block proof, same claim chain as
demo_llama7b.py with every dimension shrunk (d, d_ff, d_h, SEQ, vocab, and
the Ligero commitment itself) so a full prove + verify round-trip runs in
seconds instead of minutes. Only the size constants below and CFG differ
from demo_llama7b.py; the claim chain, scale cascade, and prove/verify
...
_s.path.insert(0, str(_R / "prover"))
_s.path.insert(0, str(_R / "demo"))
...
# Toy-scale Ligero commitment (default production CFG is ELL=8192/K_DEG=16384/
# N_LIG=65536 — massive overkill for a witness this small). T_QUERIES still
# overridable via env; 16 here is fast/non-sound, production soundness uses 80.
CFG = LigeroConfig(
    ELL=512, K_DEG=1024, N_LIG=4096, T_QUERIES=int(os.environ.get("LIGERO_T_QUERIES", "16"))
)


SEQ = 4  # overridable via --seq
d = 16
d_ff = 32
d_h = 8  # head dim (used for RoPE); H = d/d_h = 2 heads
...
def _rand_small(*shape, lo=0, hi=1000):
    return torch.randint(lo, hi, shape, dtype=torch.int64, device="cuda").to(torch.uint64)
...
def _commit_weights_from_hf(tape, model_id: str, layer_idx: int) -> Dict[str, object]:
    ...
    return {
        ...
        "rms_pre_attn_w": tape.commit(f"rms_pre_attn_w{sfx}", w["rms_pre_attn_w"], (d,)),
        "rms_pre_ffn_w": tape.commit(f"rms_pre_ffn_w{sfx}", w["rms_pre_ffn_w"], (d,)),
    }
...
def _commit_weights_from_hf_lazy(tape, lazy_loader, layer_idx: int) -> Dict[str, object]:
    ...
    for short, (hf_name, transpose, divide_by) in specs.items():
        ...
        loader = lazy_loader.make_loader(hf_name, transpose=transpose, divide_by=divide_by)
...
def _run_block(tape, x, weights, *, H: int):
    ...
    norm1_g = tape.hadamard_broadcast(
        norm1, rms_pre_attn_w, SEQ=SEQ, d=d, s_a=S, s_b=S, s_out=S, output_width=OUTPUT_WIDTH
    )
    ...
    q_rope = tape.rope(q, SEQ=SEQ, d_h=d_h, heads=H, s_x=S, s_out=S, output_width=OUTPUT_WIDTH)
    k_rope = tape.rope(k, SEQ=SEQ, d_h=d_h, heads=H, s_x=S, s_out=S, output_width=OUTPUT_WIDTH)
    ...
    attn_out = tape.matmul(
        sm_scores, v, heads=H, head_dim=SEQ, s_a=S, s_b=S, s_out=S, output_width=OUTPUT_WIDTH
    )
    ...
    norm2_g = tape.hadamard_broadcast(
        norm2, rms_pre_ffn_w, SEQ=SEQ, d=d, s_a=S, s_b=S, s_out=S, output_width=OUTPUT_WIDTH
    )
...
def _run_tail(
    ...
    *,
    ...
    final_norm_g = tape.hadamard_broadcast(
        final_norm, final_norm_w, SEQ=SEQ, d=d, s_a=S, s_b=S, s_out=S, output_width=OUTPUT_WIDTH
    )
    logits = tape.matmul(final_norm_g, W_lm_head, s_a=S, s_b=S, s_out=lm_s_out, output_width=lm_ow)
    return logits
...
def _run_unexplained_info(
    tape, logits, *, vocab_size, seq, output_tokens=None, s_c=UI_S_C, sum_positions=None
):
    ...
    k = s_c // s_b
    print(
        f"  unexplained-info (folded): gap_max={gap_max:,} (exp+gap tables), "
        f"s_c={s_c}, s_y=2^{UI_S_Y_BITS}, s_b=2^{UI_S_B_BITS}, k={k} "
        f"(output tokens hidden)"
    )
    if gap_max.bit_length() > 26:
        print(
            f"    [skip] gap_max=2^{gap_max.bit_length()} too large for the exp table; "
            f"use a coarser LM-head scale."
        )
        return None, None

    Sz, h = prove_unexplained_info(
        tape,
        logits,
        tokens,
        T=seq,
...
def _report_unexplained_info(tape, Sz, info, *, seq, vocab_size):
    ...
    n = info.get("n", seq)
    print(f"  unexplained-information bound: U = {U:.4f} bits over {n} hidden output token(s)")
    return U
...
def main(
    *,
    ...
    if from_hf is not None:
        print(f"  weights: HF model {from_hf!r}, layers [{layer_idx}, {layer_idx + num_layers})")
        if prompt is not None:
            print(f"  prompt:  {prompt!r}")
    else:
        print(f"  weights: random signed (Glorot-ish in [-32, 32); W_Q/√d_h), {num_layers} layers")
    ...
    if from_hf is not None and (prompt is not None or token_ids is not None):
        from loader import free_model_cache, load_token_embedding, tokenize_prompt
        ...
        E_subset_data = E_full.view(-1, d).index_select(0, unique_t).contiguous().view(-1)
        ...
        x_data_init = (
            E_subset_data.view(len(unique_ids), d).index_select(0, tok_t).contiguous().view(-1)
        )
        print(
            f"  E subset: {len(unique_ids)} rows (full E would be "
    ...
    if from_hf is not None and prompt is not None:
        E_wt = tape.commit("E_embedding_subset", E_subset_data, E_subset_shape)
    ...
    if lazy_weights:
        if from_hf is None:
            raise ValueError(
                "--lazy-weights requires --from-hf (random weights are generated, not loaded)"
            )
        from loader import LazyHFLoader

        lazy_loader = LazyHFLoader(from_hf, S=S, d_h=d_h)
        print(f"  lazy weights: enabled (per-weight on-demand load from {lazy_loader.model_dir})")
    ...
    for L in range(num_layers):
        ...
        if lazy_loader is not None:
            layer_weights = _commit_weights_from_hf_lazy(tape, lazy_loader, this_layer_idx)
        elif from_hf is not None:
            layer_weights = _commit_weights_from_hf(tape, from_hf, this_layer_idx)
        else:
            layer_weights = _commit_weights_random(tape, layer_idx=this_layer_idx)
    ...
    if with_lm_head:
        if from_hf is not None:
            from loader import free_model_cache, load_final_weights
            ...
            vocab_size = 64  # toy vocab (production random-weights path uses 32000)
    ...
    if unexplained_info:
        ...
        ui_Sz, ui_info = _run_unexplained_info(
            ...
            vocab_size=vocab_size,
            seq=SEQ,
            output_tokens=ui_output_tokens,
            s_c=ui_s_c,
            sum_positions=ui_positions,
        )
    ...
    if ui_Sz is not None and ui_info is not None and ui_info.get("reveal_pin") is not None:
        ...
        for _v in list(tape.inputs):
            if _v.name.endswith("_mult"):
                tape.inputs[_v].zero_()
        print(
            f"  reveal: Sz = {_sz} pinned as PUBLIC bound = "
    ...
    if _dump:
        ...
        _write_proof(
            _dump,
            pr.claims_to_json(tape.claims, CFG),
...
if __name__ == "__main__":
    ...
    _ui_tokens = (
        [int(v) for v in args.ui_output_tokens.split(",")] if args.ui_output_tokens else None
    )
    main(
        ...
        prompt=_prompt,
        with_lm_head=not args.no_lm_head,
        verbose=args.verbose,
        lazy_weights=args.lazy_weights,
        engine=args.engine,
        save_logits=args.save_logits,
        time_ops=args.time_ops,
        forward_only=args.forward_only,
        dump_proof=args.dump_proof,
        unexplained_info=args.unexplained_info,
        ui_sigma=args.ui_sigma,
        ui_sy_bits=args.ui_sy_bits,
        ui_output_tokens=_ui_tokens,
    )
