"""End-to-end ACCEPT gate for the witness SPILL: build the SMALL toy transformer
(1 layer -> contains the spilled softmax + silu ops), prove with the host spill
...
SEED = b"accept-toy-spill"
C._WITNESS_SPILL_ON = True  # host-memory spill active
...
print(f"Rust verify_proof (spill ON): {'ACCEPT' if acc else 'REJECT'}")
if not acc:
    print(msg)
