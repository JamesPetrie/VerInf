"""Soundness gate for the GPU silu decomposition: proving the SAME model with
LIGERO_GPU_SILU off (numpy) vs on (GPU int64) must give a byte-identical proof.
Single process, fixed seed -> identical model; only the silu decomposition path
differs."""
import sys
...
print(f"\n{'ALL IDENTICAL — GPU silu produces a byte-for-byte identical proof' if allok else 'MISMATCH — GPU silu is UNSOUND, revert'}")
