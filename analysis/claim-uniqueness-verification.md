...
VerInf proves a bound on the *unexplained information* in an LLM's output: the bits of the generated tokens that a committed integer model, run on the committed inputs, fails to predict. A run scores well when its committed computation assigns high probability to the actual next token at each position — lower unexplained information is better. This document is about automatically checking that a given forward-pass construction (a "claim list") earns its score honestly. Two payoffs motivate that:

- **An automated admission gate for a competition or for AI agents.** The intended use is a competition to make the prover faster and more accurate at predicting tokens, where the contestants — people or AI agents iterating on the design — submit their own claim lists. Every submission must be checked for cheating before it can be scored, and hand-proving each one's soundness is the bottleneck that would otherwise gate the whole competition on human review. An automated check removes the reviewer from the loop, so untrusted authors can submit freely and an agent can iterate in a tight loop.
- **Higher confidence in the system's security.** Even for the claims we wrote ourselves, a machine check that the prover has no room to game the score is far stronger evidence than prose lemmas — which the authors, being too close to the construction, can and did get wrong (the pre-fix RMSNorm bracket shipped unsound before it was caught). The check turns a soundness argument into something reproducible rather than a matter of careful reading.

**Why exactly these two properties.** The gate verifies two facts about a claim list: that every value is *uniquely pinned* — the prover has no witness freedom (§1) — and that the computation is *causal* — a later token cannot influence an earlier position (§6). Together they force each output logit distribution to be a **deterministic function of only the tokens that precede the one it is predicting**: no free parameters left to tune the score, and no peeking at the answer. On top of that sits the scoring step (the surprisal / unexplained-information computation, B.7), which is fixed and already locked in, and which rests on the Gibbs inequality — so the reported number is a genuine upper bound, and a *low* score can only mean the committed computation really did predict the actual tokens well from their strict past. The consequence is what makes the competition meaningful: the only way to win is to build a genuinely better, faster predictor of real LLM tokens, and predicting a wide sample of real tokens plausibly requires something functionally equivalent to running the model itself — so there is unlikely to be any shortcut faster than just running it. The two properties close off every route to a good score *other* than actually predicting.
