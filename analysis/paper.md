...
* An end-to-end zero-knowledge proof of inference of a 400-billion-parameter model, verified against an independent implementation; published work had reached about 13 billion parameters.
...
* A streaming prover design that decouples peak memory from witness size, demonstrated by our implementation proving a 7.2-terabyte witness at a working set of 83.9 GB on a single machine.
* A complete transparent instantiation over Ligero and the Goldilocks field: matrix multiplication by Freivalds' check with two random projections, the nonlinearities by table lookups, top-1 mixture-of-experts routing, and a verifier that recompiles every constraint from the public claim list and trusts only its own derivation.
