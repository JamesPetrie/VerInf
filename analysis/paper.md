...
## 1. Introduction
International agreements that constrain AI development could reduce the competitive pressure to build increasingly dangerous systems, but such agreements need verification to be stable. In the companion framework [cite], a compute operator (the prover) demonstrates to an external party (the verifier) that it runs only permitted workloads, where neither party trusts the other's hardware: the verifier records cryptographic commitments to the prover's encrypted traffic and periodically challenges the prover to show that a randomly selected output is well explained by a permitted computation on the measured inputs. Three properties of this setting shape the design. Only sampled outputs are ever proven, so proving cost amortizes over the workload it deters rather than gating it. The verifier is a state or its agent, so substantial verification computation and large proofs are acceptable, while a small trusted base and the absence of a trusted setup are essential. And the two parties are in live contact, so the proof can be interactive: the prover commits before each challenge is drawn and cannot grind over challenges offline, which lets a modest per-challenge soundness carry real deterrent weight. The framework gives three confidentiality-preserving ways to perform the recomputation; this paper builds the zero-knowledge one and demonstrates it at the scale of a frontier model.

VerInf produces a zero-knowledge proof that a committed model, run on committed inputs, well explains committed outputs. The proof reveals nothing about the weights, the activations, or the input and output tokens; its one public quantity is a bound on the unexplained information in the output, the output bits that a permitted computation on the measured inputs does not account for. A small verifier, running its own code, reads the proof and answers whether it is valid. In a compute agreement the bound caps the bandwidth available for hidden work such as covert training; the same proof supports detecting exfiltration of model weights, auditing that a deployed model matches an approved version, and checking high-stakes inference for bugs or tampering.

Deployed inference typically runs in floating point and is nondeterministic across executions, and how easily a given serving stack could be made bit-exact is difficult to judge from outside. VerInf requires no changes to the deployment: rather than reproduce its output, the proof certifies how well a committed integer version of the model predicts it. The prover supplies its own predictor of each output token, and the proof certifies the resulting bound; a poor predictor inflates only the prover's own reported number, so the choice can be left to the side best placed to make it accurate. A deployment that does run deterministically needs no new machinery and simply certifies a tighter bound. Witness slack is a liability for any inference proof: in an exact-reproduction proof, adversarially chosen slack at every layer accumulates in ways that are hard to bound, and for a bound on unexplained information even isolated slack near the logits is a direct lever for deflating it. VerInf therefore leaves the prover no deflating freedom anywhere, pinning every forward-pass claim to a unique witness, a requirement §2 makes precise and the claim design of §4 is built to satisfy.

We report, to our knowledge, the first zero-knowledge proof of inference of a frontier-scale model. VerInf produced a proof of a 1093-token forward pass of the full Llama-4-Maverick, a 400-billion-parameter mixture of experts with all 128 experts committed per MoE layer, in 19.3 hours on a single consumer NVIDIA DGX Spark; a Rust verifier that shares no code with the prover accepted it. The proof certifies 0.394 unexplained bits per token over the 651-token continuation, with the 442-token prompt hidden throughout. Published zero-knowledge inference work has previously reached about 13 billion parameters (Sun, Li, and Zhang 2024 report 13 minutes for LLaMA-2 13B at 2,048 tokens on an A100), roughly thirty times smaller, and proves a different statement: it requires the computation to match its integer circuit exactly, so it attests to a computation contrived to fit the proof rather than to a deployment as it runs. VerInf's commitments are hash-based only, so it needs no trusted setup and is plausibly post-quantum.

The committed witness for the demonstrated run is about 7.5 terabytes, sixty times the memory of the machine that proved it. The prover therefore streams the witness one operation at a time, and peak memory tracks the working set of a single operation, 83.3 GB, rather than the witness or the proof. A cost model validated against the measured runs shows the quadratic attention witness dominating the proof beyond roughly 11,000 tokens and projects proof times of days on an NVL72-class cluster at a context of one million tokens; the projection assumes dense attention in every layer, and windowed or sparse patterns reduce the quadratic term in proportion to the layers and span they cover (§8).

Our contributions are:
* The first end-to-end zero-knowledge proof of inference of a frontier-scale model, accepted by a verifier that shares no code with the prover.
* A streaming prover that decouples peak memory from witness size, proving a 7.5-terabyte witness at a working set of 83.3 GB on a single machine.
* A complete transparent instantiation over Ligero and the Goldilocks field: matrix multiplication by Freivalds' check with two random projections, the nonlinearities by table lookups, top-1 mixture-of-experts routing, and a small Rust verifier that recompiles every constraint from the public claim list and trusts only its own derivation.
* The realization of the unexplained-information bound as claim types on a general proof system for LLM inference, meeting a two-tier soundness requirement: upstream of the logits every claim admits a unique witness, and downstream every prover freedom provably inflates the reported bound.
* A cost model for the prover, fitted to measured hardware primitives and validated against the demonstrated runs.

The rest of the paper follows the proof from model to verdict. Section 2 states what VerInf proves and what the bound requires of the proof system. Section 3 compares with prior work. Section 4 compiles a model to constraints, Section 5 gives the Ligero argument and its soundness, and Section 6 describes the streaming prover and the small verifier. Section 7 reports the results, Section 8 the cost model and scaling, and Section 9 the limitations and future work.
...
**Llama-4-Maverick, 1093 tokens.** The full 48-layer, 400B-parameter mixture of experts, with all 128 experts committed per MoE layer, proven in the four-round protocol of §5.3 with 40 columns opened. The proof took 19.3 hours to generate at a prover peak of 77.6 GB GPU memory (83.3 GB unified); the committed witness is about 7.5 terabytes, streamed at the working set (§6.1). The verifier accepted the 92 GB proof, checking 30 of the 40 opened columns, in 14.0 hours on 20 CPU cores at a peak of 78.6 GB. The proof certifies a bound of $U(o) = 0.394$ bits per token over the 651-token continuation; the 442-token prompt stays hidden, while the continuation tokens are public in this run. **[TODO: state explicitly whether the surprisal claims of §4.6 are inside this proof, or whether this run proves the forward pass with the bound computed from the proven logits outside the circuit. If the latter, say so here and present the 7B run below as the end-to-end demonstration of the in-circuit bound.]** At 30 columns checked, the per-challenge soundness bound is about $2^{-12.5}$ (§5.4). Raising the configuration to 80 opened columns is a deployment choice whose measured cost is verification runtime, since the per-column work grows with the count (a GPU verifier is the identified path, §9); verifier memory is dominated by parsing the opened columns and does not depend on how many are checked.

**Llama-2-7B, 1000 tokens.** All 32 layers on real checkpoint weights: the forward-pass proof generates in about 46 minutes at a prover peak near 12 GB with the weights streamed from disk, producing a 1.4 GB proof at 10 opened columns that the verifier accepts in about 13 minutes on 20 CPU cores. A separate 50-token generation with the surprisal claims of §4.6 in the proof certified $U(o) = 0.132$ bits per token at a calibrated $\sigma$ [confirm], with the output tokens committed and hidden throughout.

**Token binding.** **[TODO: state whether the runs above include the binding of Appendix E, which postdates parts of this work. If not: report a run demonstrating the binding end to end, and state here that the headline bounds are over committed streams not yet anchored to a recorded transcript (§2.4). If so: state the provenance of the recorded digests, per the root-of-trust assumption of Appendix E.2.]**
...
The totals are validated at full scale: the verifier's independent compile of the 1093-token proof yields witness and quadratic row counts within 1% of $W(1093)$ and $Q(1093)$ (Appendix A.2).
...
Wall-clock time follows from these drivers through a per-primitive cost identity (Appendix A.5), with each heavy step priced by a measured hardware rate; the transforms are memory-bandwidth bound, so the identity's leading terms ride bandwidth, except the column hashing, which rides hash throughput. The four-round protocol multiplies only the witness recomputation, the cheapest term. At $S = 1093$ the identity gives a floor of roughly 8 to 10 hours against the measured 19.3: the implementation is within about a factor of two of its own floor, with the gap in the fold's remaining memory traffic and orchestration (Appendix C), not in the protocol.
...
- Add a linear proof linking an old weight commitment to a refreshed one, so the persistent commitment can serve many queries without exhausting its zero-knowledge budget (§5.2).
...
The three terms dominate $W$ in turn as context grows: weights below about 900 tokens, the committed experts (linear) from there to about 11,100, and attention (quadratic) above. The demonstrated 1093-token run sits at the first crossover.
...
The streaming prover never stores the witness (the encoded matrix at $S = 1093$ is 60 TB), so the four-round protocol recomputes the witness values once per round. That recomputation is the *only* cost the rounds multiply; every heavy step runs once. The cost identity is
...
- $T_{\text{wit}}$ — one witness-computation sweep (the integer forward pass and derived values), compute-bound, about an hour per pass at $S = 1093$. The only ×4 term, and the cheapest; it stays negligible at every scale.
...
Calibration against the demonstrated run: round 1 measured 10.3 ns per witness slot against its $A_c{+}D$ budget of about 5.5, and the full 19.3-hour run is within about 2× of the identity's total of 8 to 10 hours. Instrumented two-layer runs of both claim mixes (dense and mixture-of-experts; cuda-synced bucket shares) decompose the prove into: witness 29%/10%, encode 28%/46%, quadratic fold 20%/10%, linear fold 14%/21% — of which the transforms are about two-thirds and the coefficient work a quarter — column hashing 5%/8%, and auxiliary commit 3%/6%. The residual against the identity sits in the fold's last DRAM round trip and chunk orchestration — implementation, not protocol. Collapsing the identity to a single per-slot transform constant underprices the same run by about 6×: the hash, coefficient, and witness terms are load-bearing, not overhead.
...
$$
y_b^2\, S_{\text{total}} \ge \text{magic} \qquad\text{and}\qquad (y_b-1)^2\, S_{\text{total}} < \text{magic}, \qquad \text{magic} = d\, s^4 .
$$
