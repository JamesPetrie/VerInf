...
We report, to our knowledge, the first zero-knowledge proof of inference of a frontier-scale model. VerInf produced a proof of a 1000-token forward pass of the full Llama-4-Maverick, a 400-billion-parameter mixture of experts with all 128 experts committed per MoE layer, in 14.3 hours on a single consumer NVIDIA DGX Spark; a Rust verifier that shares no code with the prover accepted it, checking all 40 opened columns. The proof certifies 0.880 unexplained bits per token over the 500-token continuation, with every token hidden: the 500-token prompt and the continuation enter the proof only as committed indicators, and the bound is the proof's only public quantity. Published zero-knowledge inference work has previously reached about 13 billion parameters (Sun, Li, and Zhang 2024 report 13 minutes for LLaMA-2 13B at 2,048 tokens on an A100), roughly thirty times smaller, and proves a different statement: it requires the computation to match its integer circuit exactly, so it attests to a computation contrived to fit the proof rather than to a deployment as it runs. VerInf's commitments are hash-based only, so it needs no trusted setup and is plausibly post-quantum.

The committed witness for the demonstrated run is about 7.2 terabytes, sixty times the memory of the machine that proved it. The prover therefore streams the witness one operation at a time, and peak memory tracks the working set of a single operation, 83.9 GB, rather than the witness or the proof. A cost model validated against the measured runs shows the quadratic attention witness dominating the proof beyond roughly 11,000 tokens and projects proof times of days on an NVL72-class cluster at a context of one million tokens; the projection assumes dense attention in every layer, and windowed or sparse patterns reduce the quadratic term in proportion to the layers and span they cover (§8).
...
* A streaming prover that decouples peak memory from witness size, proving a 7.2-terabyte witness at a working set of 83.9 GB on a single machine.
...
**Llama-2-7B, 1000 tokens.** All 32 layers on real checkpoint weights: the forward-pass proof generates in about 44 minutes at a prover peak of 11.2 GB with the weights streamed from disk, producing a 1.44 GB proof at 10 opened columns that the verifier accepts in about 23 minutes on 20 CPU cores. A separate 50-token generation with the surprisal claims of §4.6 in the proof certified $U(o) = 0.132$ bits per token at a $\sigma$ calibrated to the measured fp-int divergence, with the output tokens committed and hidden throughout (that run predates the two fixes above; a re-run on the current prover is pending).
